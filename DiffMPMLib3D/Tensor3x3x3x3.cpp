...
std::string DiffMPMLib3D::ToString(const Tensor3x3x3x3& tensor)
{
    ...
    auto LeadingSpacesfloatStr = [](float number, size_t total_width)
    {
...
#pragma omp parallel for
        for (int j = 0; j < total_width - num_chars; j++) {
            ret += " ";
        }
        ret += num_str;
        return ret;
...
#pragma omp parallel for collapse(4)
    for (int a = 0; a < 3; a++) {
        for (int b = 0; b < 3; b++) {
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    ss << LeadingSpacesfloatStr(tensor[a][b](i, j), 20) << ", ";
                }
            }
            ss << std::endl;
        }
...
#pragma omp parallel for collapse(4)
    for (int a = 0; a < 3; a++) {
        for (int b = 0; b < 3; b++) {
            for (int i = 0; i < 3; i++) {
                for (int j = 0; j < 3; j++) {
                    float d = std::fabs(A[a][b](i, j) - B[a][b](i, j));
