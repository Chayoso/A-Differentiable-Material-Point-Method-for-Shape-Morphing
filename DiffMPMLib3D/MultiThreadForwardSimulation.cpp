	...
	std::vector<std::shared_ptr<Grid>>& proxy_grids, float dt, float drag, Vec3 f_ext)
{
	...
	/*bool good_num_threads = (num_threads == 1 || num_threads == 2 || num_threads == 4 || num_threads 
...
void DiffMPMLib3D::MultiThreadMPM::Multi_P2G(const std::vector<MaterialPoint>& points, Grid& proxy_grid, size_t p_ind_start, size_t num_p, float dt, float drag)
{
	for (size_t p = p_ind_start; p < p_ind_start + num_p; p++) {
...
void DiffMPMLib3D::MultiThreadMPM::Multi_P_op_2(std::vector<MaterialPoint>& next_points, const std::vector<MaterialPoint>& curr_points, size_t p_ind_start, size_t num_p, float dt)
{
	for (size_t p = p_ind_start; p < p_ind_start + num_p; p++)
	{
		const MaterialPoint& curr_mp = curr_points[p];
