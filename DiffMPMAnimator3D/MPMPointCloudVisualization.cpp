...
std::vector<float> GetPointCloudLocalMassFields(const DiffMPMLib3D::PointCloud& mpm_point_cloud, DiffMPMLib3D::Grid& grid)
{
    ...
    std::vector<float> local_mass_field;
...
void UpdatePolyscopePointCloudMassField(polyscope::PointCloud** ps_point_cloud, std::shared_ptr<const DiffMPMLib3D::PointCloud> mpm_point_cloud, std::shared_ptr<DiffMPMLib3D::Grid> grid, float min_val, float max_val)
{
	auto positions = mpm_point_cloud->GetPointPositions();
	(*ps_point_cloud)->updatePointPositions(positions);
