...
void OptInput::ImGui()
{
    ...
    ImGui::InputFloat("grid_dx", &grid_dx);
    ...
    ImGui::InputFloat("lam", &lam);
    ImGui::InputFloat("mu", &mu);
    ImGui::InputFloat("p_density", &p_density);
    ...
    std::string control_freq_str = "Control frequency = " + std::to_string(1.0 / ((float)control_stride * dt)) + " Hz";
    ...
    ImGui::InputFloat("Initial line search step size", &initial_alpha);
    ImGui::InputFloat("Gradient convergence tolerance factor", &gd_tol);
    ImGui::InputFloat("Smoothing_factor", &smoothing_factor);
}
