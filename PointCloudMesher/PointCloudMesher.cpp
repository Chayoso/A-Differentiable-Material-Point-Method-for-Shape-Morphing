...
std::string input_path = "SPHERE_TO_CUBE\\";
std::string output_path = "Videos\\";
...
void MarchingCubesPointCloud(const std::vector<Vec3>& _points, float iso_mass, float grid_dx, Vec3 grid_min_point, Vec3 grid_max_point,
...
void mesherCallback()
{
    ...
    static float grid_dx = 1.f;
    static Vec3 grid_min_point = Vec3(-16.f, -16.f, -16.f);
    static Vec3 grid_max_point = Vec3(16.f, 16.f, 16.f);

    static float iso_mass = 0.5f;
    ...
    if (ImGui::Button("Generate meshes from point clouds"))
    {
        ...
        for (int i = 0; i < num_objs; i++)
        {
            ...
            temp_surf->setSurfaceColor(glm::vec3(1.f, 0.5f, 0.5f));
